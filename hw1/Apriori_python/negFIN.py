...
class NegFIN:
    ...
    def __write_itemsets_to_file(self, N, itemset_buffer, N_itemset_length, FIS_parent_buffer, FIS_parent_length):
        ...
        if FIS_parent_length > 0:
            ...
            for i in range(1, max):
                ...
                # itemset_string = "{"+ ','.join(itemset_string)+ "}\n"
                # # Append the count of the itemset
                # itemset_string = '{0}'.format(N.count)+"\t"+ itemset_string
    ...
    def runAlgorithm(self):
        ...
        end_time = time.time()
        time_diff = (end_time - start_time) # Total execution time of algorithm.
    ...
    def writeTask1File(self, items):
        ...
        # 如果資料夾不存在，則使用os.makedirs創建它
            ...
            print(f"資料夾 '{self.output_file}' 已創建")
        else:
            print(f"資料夾 '{self.output_file}' 已存在")
